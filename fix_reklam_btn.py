...
# 1. Add state for promotion UI
state_old = "  const [isFav, setIsFav] = useState(false);"
state_new = """  const [isFav, setIsFav] = useState(false);
...
"""
text = text.replace("  if (!ad) {", handle_func + "\n  if (!ad) {")

# 3. Import Loader2 and Star
if "Loader2" not in text:
    text = text.replace('import { MapPin, Phone, MessageCircle, Share2, Heart, Crown, AlertTriangle } from "lucide-react";', 'import { MapPin, Phone, MessageCircle, Share2, Heart, Crown, AlertTriangle, Star, Loader2 } from "lucide-react";')

# 4. Replace the promo box with the interactive one
promo_old = """          {/* Promo Box - Only visible to owner */}
          {isOwner && (
            <div className="bg-blue-600 p-6 rounded-3xl text-white shadow-lg shadow-blue-600/30 text-center">
              ...
              <button className="w-full bg-white text-blue-600 font-black py-4 rounded-xl hover:bg-blue-50 transition-colors shadow-md">
                Reklam et (Xidmətlər)
              </button>
            </div>
          )}"""

promo_new = """          {/* Promo Box - Only visible to owner */}
          {isOwner && !showPromo && (
            <div className="bg-blue-600 p-6 rounded-3xl text-white shadow-lg shadow-blue-600/30 text-center">
              <Crown className="w-12 h-12 mx-auto text-orange-400 mb-4 drop-shadow-md" />
              <h3 className="font-black text-2xl mb-2">Daha çox alıcı tap!</h3>
